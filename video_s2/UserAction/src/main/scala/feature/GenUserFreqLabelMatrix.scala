...
/**
  ...
  */
object GenUserFreqLabelMatrix extends serializable{
 def extract(p:Params): Unit ={
    ...
    val label_keys = sc.textFile(p.freq_label_pt).map{
      line =>
        val arr = line.split("\t")
        arr(0)
    }.collect().toSet
  ...
  val parser = new OptionParser[Params]("Feature"){
    opt[String]("freq_label_pt")
      .text("用户行为文件输入路径")
      .action{ (x,c) => c.copy( freq_label_pt = x) }
  ...
  case class Params(freq_label_pt:String="",
                    behavior_pt:String = "",
                    feat_pt:String = "",
                    week_id:Int = 9,
                    interval:Int = 9)
