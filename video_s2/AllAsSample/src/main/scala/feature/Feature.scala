...
trait Feature {
  ...
  def print[T](path: String,datas:Map[String,Map[Int,T]], handle: (Map[Int,T],Int,Int,String) => String): Unit ={
  ...
  def printUser[T](path: String,datas:Map[String,T], handle: (T,Int,Int,String) => String): Unit ={
    val feat_out = new PrintWriter(path)
    val keys = datas.keySet.toArray.sorted
    Range( 0,7 ).map {
      did =>
